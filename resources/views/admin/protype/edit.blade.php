    ...
    <form class="form form-horizontal" id="form-admin-add"  method="post">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-8">
                <input type="text" class="input-text" value="{{ $edit->protype_name }}" placeholder="" id="protype_name" name="protype_name">
            </div>
        </div>
        <div class="row cl">
			...
			<select class="select" name="pid" size="1">
               <option value="0">=顶级分类=</option>
                        @foreach($data as $v)
                            <option value="{{ $v['id'] }}" @if($v['id']===$edit->pid) selected @endif> {{ str_repeat("&nbsp;",$v['level']*4) }}{{ $v['protype_name'] }}</option>
                        @endforeach
			</select>
			</span>
            </div>
    ...
    $(function () {
        ...
        $("#form-admin-add").validate({
            ...
            submitHandler: function (form) {
                $("form").ajaxSubmit({
                    type: 'post',
                    url: "{{ url('admin/protype/edit') }}/{{$edit->id}}",
