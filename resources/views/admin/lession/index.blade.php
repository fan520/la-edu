...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a href="javascript:;" onclick="brandDel()"
                    ...
                    href="javascript:;" onclick="brand_add()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加课时</a></span>
    </div>
    <div class="mt-20">
    ...
    $(function () {
        ...
        $table = $('.table-sort').dataTable({
            ...
            'createdRow': function (row, data) {//当每行创建的时候执行的回调函数
                ...
                //修改第4列的图片显示
                if (data.cover_img) {
                    $row.find('td:eq(4)').html("<img src='" + data.cover_img + "' width='80'>");
                }

                //修改第6列的图片显示
                    $row.find('td:eq(6)').html("<a href='{{url('admin/lession/play')}}'><img src='{{ asset('img/video.jpg') }} ' title='播放' style='border-radius:50%;width:30px;'></a>");
...
//                    console.log(data);
    ...
    function brand_add() {
        var title = "添加课时";//弹窗标题
        var url = "{{ url('admin/course/add') }}";//弹窗的地址
        var h = "600";//弹窗高度
        var w = "800";//弹窗宽度
    ...
    function brand_edit(id) {
        var title = "修改课时";//弹窗标题
        var url = "{{ url('admin/course/edit') }}/" + id;//弹窗的地址
        var h = "600";//弹窗高度
        var w = "800";//弹窗宽度
        layer_show(title, url, w, h);
    }
